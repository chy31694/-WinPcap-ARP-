		...
		if ((res = pcap_next_ex(adhandle, &pkt_header, &pkt_data)) > 0)
		{
			if (*(unsigned short*)(pkt_data + 12) == htons(ETH_ARP))
			{
				...
				if (*(unsigned short*)(pkt_data + 20) == htons(ARP_REPLY))
				{
					...
					cout << "IP address：" << (unsigned long)(recv->ah.source_ip_add & 255) << "." << (unsigned long)((recv->ah.source_ip_add >> 8) & 255) << "." << (unsigned long)((recv->ah.source_ip_add >> 16) & 255) << "." << (unsigned long)((recv->ah.source_ip_add >> 24) & 255) << "   ";
